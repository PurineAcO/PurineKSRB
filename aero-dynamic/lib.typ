...
// 题目框
#let problem-counter = counter("example")
#let example(it) = context {
  let config = config-state.get()
  custom-block(
    title: [
      #problem-counter.step()
      Example #context problem-counter.display().
    ],
...
#let make-title(
  ...
  // 作者
  if author != "" {
    set text(config.author-size, font: "KaiTi")
    align(center, author)
  }
...
#let project(
  ...
  // 图表样式
  show figure: set text(font: config.caption-font,size:9pt)
